...
SPEAKER_MODEL_NAME = "AA_Youtube" # CHANGE THIS TO YOUR NEEDS. THIS WILL BE PART OF THE MODEL NAME AFTER THE PREFIX

MODEL_DIR_PREFIX ="PortaSpeech_" # DON'T CHANGE THIS
...
def run(gpu_id, resume_checkpoint, finetune, model_dir, resume, use_wandb, wandb_resume_id):
    ...
    if model_dir is not None:
        ...
        save_dir = os.path.join(MODELS_DIR, MODEL_DIR_PREFIX + SPEAKER_MODEL_NAME)  # RENAME TO SOMETHING MEANINGFUL FOR YOUR DATA
    ...
    french_datasets.append(prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_test_multispeaker("AA"),
                                                     corpus_dir=os.path.join(PREPROCESSING_DIR, "AA_multispeaker"),
                                                     lang="fr"))  # YOU CAN SIMPLY ADD MODE CORPORA AND DO THE SAME, BUT YOU DON'T HAVE TO, ONE IS ENOUGH
    
    
    french_datasets.append(prepare_fastspeech_corpus(transcript_dict=build_path_to_transcript_dict_test_singlespeaker_AA(),
                                                     corpus_dir=os.path.join(PREPROCESSING_DIR, "AA_singlespeaker"),
                                                     lang="fr"))  # YOU CAN SIMPLY ADD MODE CORPORA AND DO THE SAME, BUT YOU DON'T HAVE TO, ONE IS ENOUGH
    

    

    all_train_sets.append(ConcatDataset(french_datasets))
    ...
    train_loop(net=model,
               ...
               eval_lang="fr",  # THE LANGUAGE YOUR PROGRESS PLOTS WILL BE MADE IN
               lr=0.00001,
               warmup_steps=500,
               ...
               resume=resume,
               phase_1_steps=1000, # GOOD RESULTS WERE OBTAINED FOR 1000 AND 5000 STEPS. JUST MAKE TESTS AND COMPARE!
               phase_2_steps=0, # Phase 2 DOES NOT SEEM TO BRING ANYTHING REGARDING VOICE CLONING
               use_wandb=use_wandb)
    if use_wandb:
        wandb.finish()
