...
def read_texts(model_id, sentence, filename, device="cpu", language="en", speaker_reference=None, duration_factor = 1.0):
    ...
    if type(sentence) == str:
        sentence = [sentence]
    tts.read_to_file(text_list=sentence, file_location=filename, duration_scaling_factor= duration_factor)
    del tts
...
def test_prononciation(version, model_id="Meta", exec_device="cpu", duration_factor = 1.0):
    os.makedirs("audios", exist_ok=True)

    read_texts(model_id=model_id,
               sentence=[
                   "Il sait que le sein de Maman n'est pas le sien.",
                   "Il peint avec la peinture un bout de pain de la boulangerie, accroché à un pin des Landes.",
                   "Est-ce un humain au juste ou juste le sien ou le mien ?",               
                    "Le sein de Maman",
                    "Le sein de Maman",
                    "Le sein nu de Maman",
                    "Le saint tesprit est sain d'esprit!"
                    "Le sein, le sien, le mien, le tiens",
                    "Le sein nu de Maman.",
                    "Je te fais un dessin pour te montrer que je suis un humain. Es-tu d'accord ?",
                    "Si c'est le cas, répond OUI!",
                    "Si ce n'est pas le cas, pourquoi ?",
                    "Et si tu le souhaites, on peut faire un bout de chemin ensemble pour en discuter!",
                    "Pas besoin de vous faire un dessin.",
                    "L'erreur est humaine pour un humain.",                               
                ],
                duration_factor= duration_factor,
               filename=f"audios/Recup_{version}.wav",
               device=exec_device,
               language="fr",
               speaker_reference=None)               
...
if __name__ == '__main__':
    ...
    # DURATION FACTOR IS USED TO STRETCH THE AUDIO. THE BIGGER IT IS THE LONGER THE AUDIO OUTPUT WILL BE (SET IT BETWEEN 0.8 [QUICKER] AND 1.2 [SLOWER])

   
    test_prononciation(version="wav_filename", model_id="AA_Youtube", duration_factor=1.2, exec_device=exec_device)
